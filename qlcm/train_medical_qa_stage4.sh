#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-4 dataset.
# Medical-coding focus (SNOMED single-label + ICD-10 multi-label).
# No replay — retention will be folded in by the user separately.
#
# Self-contained experiment — NOT the same as train_fully_async_no_math.sh.
# Differences from no_math:
#   - Dataset:        data/qlcm/medical_qa_stage4/train.parquet
#                       (2 pools: SNOMED single-label train + ICD multi-label
#                        train, both stamped with extra_info.task_type so the
#                        qa_openrouter_bench dispatcher routes to the right
#                        deterministic scorer)
#   - Reward fn:      qlcm/rewards/qa_openrouter_bench.py — OpenRouter
#                       judge + benchmark extensions including the new
#                       deterministic icd_multi_label / snomed_single_label
#                       modes. These two modes never call the judge (cheap).
#                       All output reward dicts are normalised to the same
#                       23-key union (was 18; +5 for icd P/R/F1 + snomed flags).
...
#                       + QA_JUDGE_MODEL (OpenRouter "<provider>/<model>" id).
#
...
# ---- data ----
# Stage-4 training mix — SNOMED single-label + ICD multi-label coding,
# with retention folded in by build_medical_qa_stage4_retention.py.
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage4/train.parquet]"}
# Stage-specific val set. The pipeline's builders do not emit one, but the
# artifacts exist and are task-matched: stage 3 validates on replay+benchmark
# rows, stage 4 on pure coding. Validating a coding stage against the 48-row
# medical-QA split measures the wrong task entirely.
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage4/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-QLCM}
EXP_NAME=${EXP_NAME:-medical-qa-stage4-coding}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage4_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage4_val}
...
# ---- reward dispatcher ----
# Stage-4 mixes coding (SNOMED + ICD) with retention pulled from earlier
# stages — general (chat / ifeval / safety), medical (medical_qa replay),
# and benchmarks (mcq / numeric / medec / text). The mix dispatcher routes
# medical/benchmark/coding rows to qa_openrouter_bench and routes the
# general-retention rows to the existing qlcm per-source scorer
# (chat / safety / ifeval / identity), padding every output to the same
# 25-key union (23 reward/* keys + prompt_uid + data_source_code).
...
# ---- judges ----
# Two judge endpoints — same pattern as stage-1/stage-2:
#   * Medical / bench / coding rows (qa_openrouter_bench) → OpenRouter.   Reads QA_JUDGE_*
#   * Retention rows (qlcm per-source: chat/safety/identity) ->
#     vLLM-served judge (or OpenRouter-served).                            Reads QLCM_JUDGE_*
# Required keys must come from runtime_env.yaml (gitignored).

# OpenRouter (qa_openrouter_bench). Resolution order inside the module:

#!/usr/bin/env bash
# QLCM stage 9b — stage 9 continued with the budget on the THINK BLOCK only.
# Identical to qlcm/train_lcpo_stage9.sh (same data, budgets, schedule, reward
# constants, min-think gate) except:
#   * starts from the stage-9 step-300 merged model, not stage 8;
#   * LCPO_LENGTH_TARGET=think: the length the reward controls is n_think, the
#     answer body is unconstrained;
#   * the answer-body leak guard is wired but OFF (LCPO_ANSWER_CAP_ENABLE=0), so
#     the run measures whether reasoning relocates into the answer before the
#     guard ever fires. Its per-source reference table is the free-mode answer
#     length (p90) at the end of stage 9; flip the switch to continue from a
#     checkpoint with the guard on.
#
# Leak instrumentation (no config needed): the trainer logs
#   critic/lcpo/leak/answer_ratio               mean answer tokens, budgeted / free rows
#   critic/lcpo/leak/budget_le256/answer_ratio  same, per budget bin
#   critic/lcpo/leak/answer_over_1p5x_free      share of budgeted answers > 1.5x the free mean
#   critic/lcpo/src_<id>/leak/...               the same per source
# and val-aux/<src>@bNNNNN/n_answer/mean@1 vs @free. Paired per-prompt view:
#   python3 -m full_mix.lcpo.leak_report dumps/qlcm/medical_qa_stage9b_val/<step>.jsonl
#
# Reward otherwise: task score x clip(alpha_eff * (N - n_think) + delta, 0, 1)
# for budgeted rows; task score alone for free rows; runaway -1.
#
# Run INSIDE the container (bash dev/dev.sh), from /workspace/verl:
#   bash qlcm/train_lcpo_stage9b.sh
...
# ---- model ----
MODEL_PATH=${MODEL_PATH:-/data/abdelrahman/verl/checkpoints/QLCM/medical-qa-stage9-lcpo/global_step_300/merged_hf_model}
...
EXP_NAME=${EXP_NAME:-medical-qa-stage9b-lcpo-think}
...
LCPO_LENGTH_TARGET=${LCPO_LENGTH_TARGET:-think}
...
# ---- answer-body leak guard (OFF for this run; wired for the follow-up) ----
# With the budget on the think block, an answer body longer than
# max(FLOOR, MULT x ref) tokens scales the task score linearly to 0 at twice
# that, budget rows only. `ref` is the source's free-mode answer length, p90,
# measured on stage 9's last 20 training steps (validation-only sets from the
# step-300 validation round). With ENABLE=0 the guard does nothing but the
# answer_cap / answer_over_cap / answer_cap_mult keys are still logged.
LCPO_ANSWER_CAP_ENABLE=${LCPO_ANSWER_CAP_ENABLE:-0}
LCPO_ANSWER_CAP_MULT=${LCPO_ANSWER_CAP_MULT:-1.5}
LCPO_ANSWER_CAP_FLOOR=${LCPO_ANSWER_CAP_FLOOR:-64}
LCPO_ANSWER_CAP_REF_BY_SOURCE=${LCPO_ANSWER_CAP_REF_BY_SOURCE:-"{mlb:118,ml:194,if:72,snomed_ml:926,snomed_if:380,sl:9,snomed_sl:27,slb:8,medical_qa:452,medical_conv:411,local/dolci-ifeval-32b:128,local/dolci-chat-32b:902,local/safety-dpo-reference:841,local/avey-identity:323,google/IFEval:437,openai/gsm8k:120,HuggingFaceH4/MATH-500:417}"}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-${DUMP_ROOT}/medical_qa_stage9b_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-${DUMP_ROOT}/medical_qa_stage9b_val}
...
python3 -m full_mix.main_ppo \
  ...
  +reward.reward_kwargs.lcpo.answer_cap_enable="${LCPO_ANSWER_CAP_ENABLE}" \
  +reward.reward_kwargs.lcpo.answer_cap_mult="${LCPO_ANSWER_CAP_MULT}" \
  +reward.reward_kwargs.lcpo.answer_cap_floor="${LCPO_ANSWER_CAP_FLOOR}" \
  "+reward.reward_kwargs.lcpo.answer_cap_ref_by_source=${LCPO_ANSWER_CAP_REF_BY_SOURCE}" \

...
`qlcm/common/judge_client.call_judge` uses (same response_format,
...
exported, the QLCM_JUDGE_* ones are auto-loaded from runtime_env.yaml.

Examples
--------
  # 100 requests, 16 in flight (matches QLCM_JUDGE_CONCURRENCY)
  python qlcm/diagnose_openrouter.py -n 100 -c 16

  # isolate whether response_format/json_object is the culprit
  python qlcm/diagnose_openrouter.py -n 50 --no-json
  python qlcm/diagnose_openrouter.py -n 50 --no-thinking

  # pull OpenRouter's per-generation stats (provider, native finish reason)
  python qlcm/diagnose_openrouter.py -n 30 --gen-stats
"""
...
def _autoload_env() -> None:
    """Populate missing QLCM_JUDGE_* vars from runtime_env.yaml.
    ...
    with open(_RUNTIME_ENV) as f:
        for line in f:
            ...
            if key.startswith("QLCM_JUDGE_") and not os.environ.get(key):
                os.environ[key] = val
...
def build_kwargs(args, model: str) -> dict:
    ...
    force_json = (not args.no_json) and os.environ.get("QLCM_JUDGE_FORCE_JSON", "1") != "0"
    if force_json:
        kwargs["response_format"] = {"type": "json_object"}
    disable_thinking = (not args.no_thinking) and os.environ.get(
        "QLCM_JUDGE_DISABLE_THINKING", "1"
    ) != "0"
    if disable_thinking:
...
def main() -> int:
    ...
    ap.add_argument("-c", "--concurrency", type=int,
                    default=int(os.environ.get("QLCM_JUDGE_CONCURRENCY", 16)))
    ap.add_argument("--model", default=os.environ.get("QLCM_JUDGE_MODEL"))
    ap.add_argument("--max-tokens", type=int,
                    default=int(os.environ.get("QLCM_JUDGE_MAX_TOKENS", 512)))
    ap.add_argument("--timeout", type=float,
                    default=float(os.environ.get("QLCM_JUDGE_TIMEOUT", 60)))
    ...
    if not api_base or not args.model:
        print("ERROR: QLCM_JUDGE_API_BASE and QLCM_JUDGE_MODEL must be set "

#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-3 dataset.
# Medical replay + benchmarks — single judge endpoint.
#
# Self-contained experiment — NOT the same as train_fully_async_no_math.sh.
# Differences from no_math:
#   - Dataset:        data/qlcm/medical_qa_stage3/train.parquet
#                       (4 pools: stage-1 mastered replay + stage-2 unmastered
#                        + stage-2 mastered + new benchmarks parquet with 4
#                        bench eval_modes, all in the medical_qa_compat schema)
#   - Reward fn:      qlcm/rewards/qa_openrouter_bench.py — OpenRouter
#                       judge + benchmark extensions (mcq / numeric /
#                       medec_hybrid / text_judge_conv). MCQ and numeric
#                       paths are deterministic — no judge call. All output
#                       reward dicts are normalised to the same 18-key union.
...
# ---- data ----
# Stage-3 training mix — replay + benchmarks (built by build_medical_qa_stage3.py).
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage3/train.parquet]"}
...
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage3/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-QLCM}
EXP_NAME=${EXP_NAME:-medical-qa-stage3}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage3_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage3_val}
...
# ---- reward dispatcher ----
# Stage-3 mixes medical replay (routes through the original qa/conversation
# paths) AND benchmark rows (route through one of the four new bench eval_modes:
# mcq / numeric / medec_hybrid / text_judge_conv). qa_openrouter_bench wraps
# qa_openrouter with that dispatch + a final reward-dict normaliser so every
# row produces the same 18-key union.
REWARD_FN_PATH=${REWARD_FN_PATH:-/workspace/verl/qlcm/rewards/qa_openrouter_bench.py}
REWARD_FN_NAME=${REWARD_FN_NAME:-compute_score}


# ---- judge ----
# Single judge endpoint: medical samples go through qa_openrouter.
# Required keys must come from runtime_env.yaml (gitignored).
#
# OpenRouter API-key resolution inside qa_openrouter:
...
# To switch to direct OpenAI: set REWARD_FN_PATH=.../qa_openai.py and export
# QA_JUDGE_OPENAI_API_KEY (or OPENAI_API_KEY) + QA_JUDGE_MODEL=gpt-5.4-mini.
# To switch back to qa_bedrock: set REWARD_FN_PATH=.../qa_bedrock.py and
# export QA_JUDGE_AWS_ACCESS_KEY / _SECRET_KEY / _REGION instead.

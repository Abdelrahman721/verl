...
The dispatcher (qlcm/rewards/compute_score.py) ALSO applies the
...
Notes:
  - ``ground_truth`` carries the first ``reference_responses`` entry from
    the source dataset (see ``qlcm.preprocess.identity_train``).
...
from qlcm.rewards.identity_judges import reference_aligned_identity_score

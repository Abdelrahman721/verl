...
qlcm's threadpool batching all run concurrently on their own subsets),
and results are reassembled in original order.

Wire in the training script with:
    REWARD_FN_PATH=/workspace/verl/qlcm/rewards/compute_score_coding_mix.py
...
from qlcm.rewards import coding as _coding  # noqa: E402
...
_PADDING_DEFAULTS: dict = {
    # length budget penalty keys; every row must carry them so the
    # DataProto key union stays consistent across families.
...
def _compute_batch(*args, **kwargs):
    ...
    if other_idx:
        _scatter(_qlcm_cs.compute_score(

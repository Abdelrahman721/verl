...
# Make `qlcm` importable whether we're loaded as a script (stage 3 points
# reward.custom_reward_function.path straight at this file) or as a package
# (the stage-1/4/5/6/8 mixes import it) — same shim the mix dispatchers use.
...
# LOGGING
# ============================================================================
log = logging.getLogger("qa_v2_reward_openrouter_bench")
if not log.handlers:
...
# ---- optional: drop the never-read justification fields from the QA verdict ---
# The reward reads only accuracy, clarity and key_point_coverage. The three
# *_justification fields are generated, billed as output tokens, and discarded.
# Output is ~94% of this judge's cost once OpenRouter's prompt cache warms, so
# removing them cuts the QA judge's bill ~28%.
#
# KEPT deliberately: the *_category fields (they precede each score and scaffold
# it) and key_point_coverage (it drives _compute_completeness).
#
# Validated before enabling, on 45 real QA rollouts stratified across the score
# range (15 per band) x 4 repeats, paired per row against the untrimmed schema:
#     accuracy POOLED  +0.100 [-0.073, +0.273]   no detectable effect
#       low 0-3        -0.150 [-0.357, +0.057]
#       mid 4-7        +0.283 [-0.087, +0.653]
#       high 8-10      +0.167 [-0.107, +0.440]
#     clarity  POOLED  -0.061 [-0.163, +0.041]   no detectable effect
# against a judge whose own self-inconsistency at temperature 0 is ~1.8 points.
# An earlier, underpowered run (n=20, borderline rows only) reported a
# significant -0.287 on accuracy; it did not reproduce under stratification and
# was a false positive. One of eight band-by-field comparisons (clarity, high
# band) came out -0.133 [-0.239, -0.028]; that is about what chance predicts
# across eight tests and is a tenth of the judge's own noise.
#
# Set QLCM_JUDGE_TRIM_QA=0 to restore the full schema without a code change.
if os.environ.get("QLCM_JUDGE_TRIM_QA", "1").strip().lower() not in ("0", "false", "no", ""):
    for _f in ("accuracy_justification", "clarity_justification", "overall_justification"):
        QA_RESPONSE_SCHEMA["properties"].pop(_f, None)
        if _f in QA_RESPONSE_SCHEMA.get("required", []):
            QA_RESPONSE_SCHEMA["required"].remove(_f)
...
# ---- optional: drop the never-read justification fields from the conv verdict --
# The reward reads only accuracy/behavior/compliance. The four *_justification
# fields are generated, billed as output tokens, and thrown away. Output is
# ~94% of this judge's cost (input is nearly free once OpenRouter's prompt cache
# warms), so removing them cuts the conversation judge's bill ~45%.
#
# The *_category fields are deliberately KEPT: they precede each score in the
# schema and act as scaffolding for it. The justifications follow their score
# and cannot influence a number already emitted.
#
# Validated before enabling, on 20 real conversation rollouts x 4 repeats,
# paired per row against the untrimmed schema:
#     accuracy   +0.188 [-0.114, +0.489]   no detectable effect
#     behavior   +0.013 [-0.103, +0.128]   no detectable effect
#     compliance -0.037 [-0.220, +0.145]   no detectable effect
# against a judge whose own self-inconsistency at temperature 0 is ~1.8 points.
#
# Set QLCM_JUDGE_TRIM_CONV=0 to restore the full schema without a code change.
if os.environ.get("QLCM_JUDGE_TRIM_CONV", "1").strip().lower() not in ("0", "false", "no", ""):
    for _f in ("accuracy_justification", "behavior_justification",
               "compliance_justification", "overall_justification"):
        CONV_RESPONSE_SCHEMA["properties"].pop(_f, None)
        if _f in CONV_RESPONSE_SCHEMA.get("required", []):
            CONV_RESPONSE_SCHEMA["required"].remove(_f)
...
def _score_single(solution_str, ground_truth, extra_info, judge_result, eval_mode):
    ...
    # New: benchmark extensions — dispatch before the original qa/conv branches.
    # Returns None if eval_mode is not a benchmark mode (then falls through).
    _bench_out = _bench_score_dispatch(
        eval_mode, solution_str, ground_truth, extra_info,
        judge_result, format_penalty, reasoning_len, answer_len, answer_to_grade,
    )
...
def _determine_eval_mode(data_source, extra_info):
    if extra_info and isinstance(extra_info, dict):
        ...
        if mode:
            return mode
        # New: benchmark extension — route on task_type / helm_scenario
        bench_mode = _bench_determine_eval_mode(extra_info)
        if bench_mode:
            return bench_mode
...
def compute_score(
    ...
    **kwargs,
    ...
    if solution_strs is not None:
        out = _compute_score_batch(data_sources, solution_strs, ground_truths, extra_infos)
        for _r, _s in zip(out, solution_strs):
            _length_penalty.apply(_r, _s)
        return out
    else:
        out = _compute_score_single(data_source, solution_str, ground_truth, extra_info)
        return _length_penalty.apply(out, solution_str)


def _compute_score_single(data_source, solution_str, ground_truth, extra_info=None):
    # Capture the original (pre-parse) ground_truth so we can emit it in the
    # rollout dump verbatim via the `reward/gold` workaround — see the
    # docstring on `_serialize_gold` for the why.
    raw_gt = ground_truth
    ...
    judge_result = None
    # Skip the judge call entirely for deterministic modes (mcq, numeric)
    needs_judge = eval_mode in ("qa", "conversation") or _bench_needs_judge(eval_mode)
    if answer_to_grade and needs_judge:
        try:
        ...
        elif eval_mode in ("medec_hybrid", "text_judge_conv"):
            # Benchmark text modes use the conv judge with a synthesized GT
            gt_args = _bench_judge_args(eval_mode, ground_truth, extra_info,
                                         answer_to_grade, solution_str)
            coro = _judge_one("conversation", *gt_args)
        else:
    ...
    out = _normalize_reward_dict(
        _score_single(solution_str, ground_truth, extra_info, judge_result, eval_mode)
    )
    # Inject the row's gold so it appears in the rollout JSONL even though
    # the fully-async dump path can't recover `gts` from the TQ.
...
def _compute_score_batch(data_sources, solution_strs, ground_truths, extra_infos=None):
    ...
    for i in range(n):
        ...
        # Determine whether this item needs a judge call at all
        needs_judge = eval_mode in ("qa", "conversation") or _bench_needs_judge(eval_mode)
        if answer_to_grade and needs_judge:
            if eval_mode == "qa":
                ...
                # Benchmark text modes — conv judge with synthesized GT
                gt_with_context = _build_text_judge_conv_gt(gt, ei)
    ...
    scores = []
    for i in range(n):
        out = _normalize_reward_dict(_score_single(
            solution_strs[i], parsed_gts[i], extra_infos[i], judge_results[i], eval_modes[i]
        ))
        # Inject the row's gold so it appears in the rollout JSONL even though
        # the fully-async dump path can't recover `gts` from the TQ.
        out["reward/gold"] = _serialize_gold(ground_truths[i])
        scores.append(out)
...
# ────────────────────────────────────────────────────────────────────────────
# BENCHMARK-DATASETS EXTENSIONS (added — backward-compatible)
# ────────────────────────────────────────────────────────────────────────────
# Adds four new eval_modes that route off `extra_info.task_type` and
# `extra_info.helm_scenario`:
#
#   "mcq"             — exact-letter match (no judge call)
#                       headqa, medmcqa, medqa, pubmedqa, medhallu
#   "numeric"         — relative-tolerance numeric match (no judge call)
#                       medcalcbench  (tolerance configurable)
#   "medec_hybrid"    — multiplicative: line# (exact) × judge_score (conv judge)
#                       medec
#   "text_judge_conv" — pure conv-judge on long-form generation
#                       aci-bench, dischargeme, meddialog, mediqa-qa, mimic-bhc
#
# Nothing in the existing `qa` / `conversation` paths is modified — old data
# still routes through the original branches unchanged.

# ── Routing tables ──────────────────────────────────────────────────────────
# Mapping by `task_type` (set on each benchmark row at parquet-build time)
_TASK_TYPE_TO_EVAL_MODE: dict[str, str] = {
    "mcq":                  "mcq",
    "classification":       "mcq",   # medhallu (binary 0/1 — treated as mcq)
    "icd_multi_label":      "icd_multi_label",      # ICD-10 P/R/F1 scorer (deterministic)
    "snomed_single_label":  "snomed_single_label",  # SNOMED single-ID scorer (deterministic)
}

# Mapping by `helm_scenario` — overrides task_type when a more specific
# verifier exists (e.g. medcalcbench is task_type="generation" but verifiable)
...
# Modes that DO need a judge API call
_JUDGE_REQUIRING_MODES = frozenset({"qa", "conversation", "medec_hybrid", "text_judge_conv"})

# Numeric tolerance for the numeric mode (relative, with absolute floor for ~0 golds)
NUMERIC_REL_TOL = float(os.environ.get("QA_NUMERIC_REL_TOL", "0.01"))   # 1% relative
NUMERIC_ABS_TOL = float(os.environ.get("QA_NUMERIC_ABS_TOL", "1e-3"))   # absolute floor

# Medec scoring: hybrid weights. If you prefer additive instead of multiplicative,
# set QA_MEDEC_LINE_WEIGHT > 0 — then reward = w*line_match + (1-w)*judge.
MEDEC_LINE_WEIGHT = float(os.environ.get("QA_MEDEC_LINE_WEIGHT", "0.0"))  # 0 → multiplicative


# ── Parsers (deterministic — no LLM) ───────────────────────────────────────
# Design: with chain-of-thought enabled the answer typically appears at the
# END of the response (after reasoning), often introduced by an explicit
# "Answer:" / "Final answer:" anchor or wrapped in `\boxed{...}`. The previous
# parsers picked the FIRST occurrence, which is almost always wrong on a
# rationale-style output. Priority now is:
#   1. Anchored matches: "answer is X", "Answer: X", "final answer = X",
#      with X allowed to be surrounded by markdown bold / parens / brackets.
#      LAST anchor wins so any restated final claim trumps earlier mentions.
#   2. \boxed{X}  (math-mode answers)
#   3. LAST standalone occurrence in the whole answer.
# This is robust to "Option B is wrong, the answer is C" and to numeric
# reasoning where the input value appears before the computed result.
_MCQ_LETTER_RE      = re.compile(r"\b([A-Ea-e])\b")
_MCQ_BINARY_RE      = re.compile(r"\b([01])\b")
_NUMERIC_RE         = re.compile(r"[-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?")
_MEDEC_LINE_RE      = re.compile(r"^\s*(\d+)\b")

# "answer is X" / "answer: X" / "final answer = X", optional markdown bold,
# optional parens / square brackets around the value.
_MCQ_ANCHOR_RE = re.compile(
    r"(?:final\s+)?answers?\s*(?:is|:|=|\s)\s*\**\s*[\(\[]?\s*"
    r"([A-Ea-e]|[01])\s*[\)\]]?\b",
    re.IGNORECASE,
)
# Same shape but the value is a number — allow internal thousands separators.
_NUMERIC_ANCHOR_RE = re.compile(
    r"(?:final\s+)?answers?\s*(?:is|:|=|\s)\s*\**\s*"
    r"([-+]?\d[\d,]*(?:\.\d+)?(?:[eE][-+]?\d+)?)",
    re.IGNORECASE,
)
# \boxed{X} — content of the last box.
_BOXED_RE = re.compile(r"\\boxed\{\s*([^}]+?)\s*\}")


def _parse_mcq_letter(answer: str) -> str | None:
    """Return the model's chosen MCQ letter (A-E) or binary digit (0/1).

    Tries explicit anchors first (LAST match wins), then `\\boxed{}`, then
    falls back to the LAST standalone letter in the response. Returns the
    canonical upper-case letter (or "0" / "1") on success, None otherwise.
    """
    if not answer:
        return None
    a = answer.strip()

    # 1. Explicit "answer is X" anchor — last occurrence wins.
    anchors = list(_MCQ_ANCHOR_RE.finditer(a))
    if anchors:
        cand = anchors[-1].group(1).upper()
        if cand in "ABCDE" or cand in "01":
            return cand

    # 2. \boxed{...} — last box wins, then first MCQ letter inside it.
    boxed = list(_BOXED_RE.finditer(a))
    if boxed:
        inner = boxed[-1].group(1).strip().upper()
        m = _MCQ_LETTER_RE.search(inner)
        if m:
            return m.group(1).upper()
        m = _MCQ_BINARY_RE.search(inner)
        if m:
            return m.group(1)

    # 3. Fall back to LAST standalone letter / binary digit in the response.
    last_let = list(_MCQ_LETTER_RE.finditer(a))
    last_bin = list(_MCQ_BINARY_RE.finditer(a))
    last_let_m = last_let[-1] if last_let else None
    last_bin_m = last_bin[-1] if last_bin else None
    # Prefer whichever appears later in the text.
    if last_let_m and (not last_bin_m or last_let_m.start() >= last_bin_m.start()):
        return last_let_m.group(1).upper()
    if last_bin_m:
        return last_bin_m.group(1)
    return None


def _parse_numeric_value(answer: str) -> float | None:
    """Extract the model's final numeric answer as a float, or None.

    Same priority as MCQ: anchored match → `\\boxed{}` → LAST number.
    Strips thousands-separator commas before parsing.
    """
    if not answer:
        return None
    cleaned = answer.replace(",", "")  # 142,500 → 142500 (medical numbers)

    # 1. Explicit "answer is X" anchor — last occurrence wins.
    anchors = list(_NUMERIC_ANCHOR_RE.finditer(cleaned))
    if anchors:
        try:
            return float(anchors[-1].group(1).replace(",", ""))
        except ValueError:
            pass

    # 2. \boxed{...} — last box wins, first number inside it.
    boxed = list(_BOXED_RE.finditer(cleaned))
    if boxed:
        inner = boxed[-1].group(1).replace(",", "")
        m = _NUMERIC_RE.search(inner)
        if m:
            try:
    ...
    # 3. Fall back to LAST number in the response (reasoning typically
    # discusses input values first then derives the result).
    last = list(_NUMERIC_RE.finditer(cleaned))
    if last:
        try:
            return float(last[-1].group(0))
        except ValueError:
            return None
    return None


def _parse_medec_answer(answer: str) -> tuple[int | None, str]:
    """Parse `<line_number> <correction text>` from `answer`.
    Returns (line_number, correction). Either field may be None / empty."""
    if not answer:
        return None, ""
    line = answer.strip().splitlines()[0] if answer.strip() else ""
    m = _MEDEC_LINE_RE.search(line)
    if not m:
        return None, line.strip()
    line_num = int(m.group(1))
    # Strip the number off the front to get the correction text
    correction = line[m.end():].strip()
    # If correction is empty on line 1, look at remaining lines
    if not correction:
        rest = "\n".join(answer.strip().splitlines()[1:]).strip()
        if rest:
            correction = rest
    return line_num, correction


def _parse_medec_gold(gold: str) -> tuple[int | None, str]:
    """Same parser, applied to the gold string."""
    return _parse_medec_answer(gold)


# ── Per-mode score functions (do not call the existing _score_single paths) ─
def _empty_score_dict(eval_mode: str, format_penalty: float,
                      reasoning_len: int, answer_len: int,
                      length_penalty: float = 0.0, length_ratio: float = 1.0) -> dict:
    """Common shell with all reward/* keys present (zeros for irrelevant dims)
    so downstream logging always finds the same column set."""
    return {
        "score": 0.0,
        ...
        "reward/reasoning_length": reasoning_len,
        "reward/answer_length": answer_len,
        "reward/eval_mode": eval_mode,
    }


def _score_mcq(solution_str, ground_truth, extra_info,
               format_penalty, reasoning_len, answer_len, answer_to_grade):
    out = _empty_score_dict("mcq", format_penalty, reasoning_len, answer_len)
    gold = (ground_truth.get("gold_answer") or "").strip()
    gold_letter = _parse_mcq_letter(gold) or gold.strip().upper()
    pred_letter = _parse_mcq_letter(answer_to_grade)
    correct = bool(gold_letter) and bool(pred_letter) and gold_letter == pred_letter
    raw = 1.0 if correct else 0.0
    out["reward/raw_score"] = raw
    out["reward/judge_score"] = raw  # for telemetry parity
    out["reward/accuracy"] = 10.0 if correct else 0.0
    out["score"] = max(0.0, raw + format_penalty)
    out["reward/correct"] = int(correct)
    return out


def _score_numeric(solution_str, ground_truth, extra_info,
                   format_penalty, reasoning_len, answer_len, answer_to_grade):
    out = _empty_score_dict("numeric", format_penalty, reasoning_len, answer_len)
    gold_str = (ground_truth.get("gold_answer") or "").strip()
    gold_val = _parse_numeric_value(gold_str)
    pred_val = _parse_numeric_value(answer_to_grade)
    if gold_val is None or pred_val is None:
        out["score"] = max(0.0, 0.0 + format_penalty)
        return out
    diff = abs(pred_val - gold_val)
    tol = max(NUMERIC_ABS_TOL, NUMERIC_REL_TOL * abs(gold_val))
    correct = diff <= tol
    raw = 1.0 if correct else 0.0
    out["reward/raw_score"] = raw
    ...
    out["score"] = max(0.0, raw + format_penalty)
    out["reward/correct"] = int(correct)
    out["reward/numeric_abs_diff"] = diff
    out["reward/numeric_rel_diff"] = (diff / abs(gold_val)) if abs(gold_val) > 0 else float("inf")
    return out


def _score_medec_hybrid(solution_str, ground_truth, extra_info,
                        format_penalty, reasoning_len, answer_len, answer_to_grade,
                        judge_result):
    """reward = line_match × judge_score   (multiplicative by default)
       reward = w*line_match + (1-w)*judge_score   (additive if QA_MEDEC_LINE_WEIGHT > 0)"""
    out = _empty_score_dict("medec_hybrid", format_penalty, reasoning_len, answer_len)
    gold = ground_truth.get("gold_answer") or ""
    gold_line, _ = _parse_medec_gold(gold)
    pred_line, _ = _parse_medec_answer(answer_to_grade)
    line_match = 1.0 if (gold_line is not None and pred_line == gold_line) else 0.0

    # Judge-side: reuse the conv-judge result (already computed before this call)
    if judge_result and judge_result.get("error") == "refusal":
        judge_score = REFUSAL_REWARD
    elif judge_result is not None and "error" not in judge_result:
        conv_reward = _compute_conv_reward(judge_result)
        judge_score = conv_reward["penalized_score"]
    else:
        judge_score = 0.0

    if MEDEC_LINE_WEIGHT > 0:
        raw = MEDEC_LINE_WEIGHT * line_match + (1 - MEDEC_LINE_WEIGHT) * judge_score
    else:
        raw = line_match * judge_score

    out["reward/raw_score"] = raw
    out["reward/judge_score"] = judge_score
    out["reward/medec_line_match"] = line_match
    out["reward/accuracy"] = _safe_float(judge_result.get("accuracy", 0), 0, "medec_acc") if judge_result else 0
    out["reward/behavior"] = _safe_float(judge_result.get("behavior", 0), 0, "medec_beh") if judge_result else 0
    out["reward/compliance"] = _safe_float(judge_result.get("compliance", 0), 0, "medec_comp") if judge_result else 0
    out["score"] = max(0.0, raw + format_penalty)
    return out


def _score_text_judge_conv(solution_str, ground_truth, extra_info,
                            format_penalty, reasoning_len, answer_len, answer_to_grade,
                            judge_result):
    """Pure conv-judge scoring for long-form generation tasks (no KP, no length)."""
    out = _empty_score_dict("text_judge_conv", format_penalty, reasoning_len, answer_len)
    ...
    out["reward/raw_score"] = judge_score
    out["reward/judge_score"] = judge_score
    out["reward/accuracy"] = REFUSAL_REWARD * 10 if is_refusal else (_safe_float(judge_result.get("accuracy", 0), 0, "txt_acc") if judge_result else 0)
    out["reward/behavior"] = REFUSAL_REWARD * 10 if is_refusal else (_safe_float(judge_result.get("behavior", 0), 0, "txt_beh") if judge_result else 0)
    out["reward/compliance"] = REFUSAL_REWARD * 10 if is_refusal else (_safe_float(judge_result.get("compliance", 0), 0, "txt_comp") if judge_result else 0)
    out["score"] = max(0.0, judge_score + format_penalty)
    return out


# ── Medical-coding scorers (ICD-10 multi-label, SNOMED single-label) ────────
# Both are DETERMINISTIC — no LLM judge call. The PARSING logic is copied
# verbatim from icd.py / snomed.py (the user's references) so any tuning to
# the regexes / thresholds happens by editing _icd_extract_answer,
# _ICD_CODE_REGEX, _SNOMED_THINK_CLOSE_PATTERN, etc. here. What the shims add
# on top is verl compatibility: signature normalisation, format_penalty
# folding, and emission of a normalised reward dict that the final
# `_normalize_reward_dict` call (entry point) pads to the union shape.

# --- ICD-10 multi-label (icd.py) verbatim regex + extraction ---
_ICD_CODE_REGEX = re.compile(r"\b[A-Z][A-Z0-9]{2}(?:\.[A-Z0-9]{1,4})?\b")


def _icd_extract_answer(model_answer: str) -> str:
    """Verbatim from icd.py — preserved as-is per design decision."""
...
def _icd_compute_multi_label_scores(solution_str: str, ground_truth) -> dict:
    """Verbatim port of icd.compute_multi_label_scores. P/R/F1 between predicted
    and gold ICD code sets. Returns {score (=F1), reward/precision,
    reward/recall, reward/f1, reward/accuracy_score (=F1)}."""
    response = _icd_extract_answer(solution_str)
    pred_codes = set(_ICD_CODE_REGEX.findall(response))
    gt_str = str(ground_truth)   # handles list / JSON string / dict-repr alike
    gt_codes = set(_ICD_CODE_REGEX.findall(gt_str))
...
def _score_icd_multi_label(solution_str, ground_truth, extra_info,
                           format_penalty, reasoning_len, answer_len, answer_to_grade):
    """Shim wrapping the icd.py multi-label scorer for the bench dispatcher.

    Applies format_penalty (raw F1 + format_penalty, clamped at 0). The output
    is built from `_empty_score_dict` then layered with ICD-specific telemetry
    (precision, recall, f1). The final `_normalize_reward_dict` at the
    compute_score entry point pads to the union key set.
    """
    out = _empty_score_dict("icd_multi_label", format_penalty, reasoning_len, answer_len)
    icd_out = _icd_compute_multi_label_scores(solution_str, ground_truth)
    raw_f1 = float(icd_out["score"])
    out["reward/raw_score"]    = raw_f1
    out["reward/judge_score"]  = raw_f1   # alias for telemetry parity w/ other modes
    out["reward/precision"]    = float(icd_out["reward/precision"])
    out["reward/recall"]       = float(icd_out["reward/recall"])
    out["reward/f1"]           = float(icd_out["reward/f1"])
    # Project F1 onto the 0-10 accuracy axis so logged dashboards line up.
    out["reward/accuracy"]     = raw_f1 * 10.0
    out["score"] = max(0.0, raw_f1 + format_penalty)
    return out


# --- SNOMED single-label (snomed.py) verbatim parsing ---
_SNOMED_THINK_CLOSE_PATTERN = re.compile(r"</think>", re.IGNORECASE)
_SNOMED_SCT_ID_PATTERN = re.compile(r"\d+")


def _snomed_extract_sct_ids(answer_text: str) -> list[str]:
    """Verbatim from snomed.py — preserved as-is per design decision."""
    return [
        tok
        for tok in _SNOMED_SCT_ID_PATTERN.findall(answer_text or "")
        if 6 <= len(tok) <= 18
    ]


def _snomed_compute_score_logic(solution_str: str, ground_truth) -> float:
    """Verbatim port of snomed.compute_score's BODY (with the broken signature
    swapped for verl-compatible inputs). Returns 1.0 (exact ID), 0.5 (label
    match without ID), or 0.0 (anything else)."""
    # snomed.py's body assumes ground_truth is a JSON string of {sct_id, label}.
    # The build pipeline preserves that shape (already JSON in the source).
    ...
    think_match = _SNOMED_THINK_CLOSE_PATTERN.search(solution_str or "")
    if not think_match:
        return 0.0
    answer_text = (solution_str or "")[think_match.end():].strip()

    extracted_ids = _snomed_extract_sct_ids(answer_text)
...
def _score_snomed_single_label(solution_str, ground_truth, extra_info,
                               format_penalty, reasoning_len, answer_len, answer_to_grade):
    """Shim wrapping the snomed.py single-label scorer for the bench dispatcher.

    Emits the same kind of telemetry the other bench scorers do
    (raw_score / judge_score parity) plus snomed-specific flags
    `reward/sct_id_match` and `reward/sct_label_match`. format_penalty is
    folded on top of the raw 0/0.5/1 score.
    """
    out = _empty_score_dict("snomed_single_label", format_penalty, reasoning_len, answer_len)
    raw = float(_snomed_compute_score_logic(solution_str, ground_truth))
    out["reward/raw_score"]       = raw
    out["reward/judge_score"]      = raw
    out["reward/sct_id_match"]    = 1.0 if raw == 1.0 else 0.0
    out["reward/sct_label_match"] = 1.0 if raw == 0.5 else 0.0
    out["reward/accuracy"]        = raw * 10.0
    out["score"] = max(0.0, raw + format_penalty)
    return out


# ── Conv-judge ground_truth assembly for text-only modes ────────────────────
# The conv judge expects: type_descriptor, context, latest_user, gold_response,
# sub_index, total_subs. For benchmark text tasks none of these apply naturally —
# we build a minimal shape from the full prompt and the gold string.
_GENERIC_TEXT_TYPE_DESCRIPTOR = (
    "CATEGORY: GENERAL MEDICAL RESPONSE\n\n"
    "The user has posed a medical question or asked the model to perform a "
    "well-defined clinical task (e.g. summarize a discharge, draft a note, "
    "produce a structured answer). Judge the candidate response against the "
    "gold reference for factual accuracy, completeness, appropriate behavior, "
    "and instruction compliance."
)

def _build_text_judge_conv_gt(ground_truth: dict, extra_info: dict) -> dict:
    """Build a conv-judge-compatible ground_truth dict from a benchmark text item.
    `extra_info.question` holds the full prompt (already includes any task
    instructions); `ground_truth.gold_answer` (or .gold_response) holds the
    target text."""
    ei = extra_info or {}
    full_prompt = ei.get("question") or ""
    gold = ground_truth.get("gold_response") or ground_truth.get("gold_answer") or ""
    return {
        "type_descriptor": _GENERIC_TEXT_TYPE_DESCRIPTOR,
        "context": "(No prior context — single-turn benchmark task)",
        "latest_user": full_prompt,
        "gold_response": gold,
        "sub_index": 1,
        "total_subs": 1,
    }
...
# ── Hooks invoked by the patched dispatchers below ─────────────────────────
def _bench_determine_eval_mode(extra_info) -> str | None:
    """Return the benchmark-extension eval_mode, or None if not a benchmark item."""
...
def _bench_needs_judge(eval_mode: str) -> bool:
    """True if this mode requires a judge API call (mcq/numeric do not)."""
    return eval_mode in ("medec_hybrid", "text_judge_conv")


def _bench_judge_args(eval_mode, ground_truth, extra_info, answer_to_grade, solution_str):
    """Build the args tuple to pass to _call_conv_judge for medec_hybrid / text_judge_conv."""
    gt_with_context = _build_text_judge_conv_gt(ground_truth, extra_info)
    return (gt_with_context, answer_to_grade, solution_str)


def _bench_score_dispatch(eval_mode, solution_str, ground_truth, extra_info,
                          judge_result, format_penalty, reasoning_len, answer_len, answer_to_grade) -> dict | None:
    """Return the score dict for the given benchmark eval_mode, or None if not handled."""
    if eval_mode == "mcq":
        return _score_mcq(solution_str, ground_truth, extra_info,
                          format_penalty, reasoning_len, answer_len, answer_to_grade)
    if eval_mode == "numeric":
        return _score_numeric(solution_str, ground_truth, extra_info,
                              format_penalty, reasoning_len, answer_len, answer_to_grade)
    if eval_mode == "medec_hybrid":
        return _score_medec_hybrid(solution_str, ground_truth, extra_info,
                                   format_penalty, reasoning_len, answer_len, answer_to_grade,
                                   judge_result)
    if eval_mode == "text_judge_conv":
        return _score_text_judge_conv(solution_str, ground_truth, extra_info,
                                       format_penalty, reasoning_len, answer_len, answer_to_grade,
                                       judge_result)
    if eval_mode == "icd_multi_label":
        return _score_icd_multi_label(solution_str, ground_truth, extra_info,
                                       format_penalty, reasoning_len, answer_len, answer_to_grade)
    if eval_mode == "snomed_single_label":
        return _score_snomed_single_label(solution_str, ground_truth, extra_info,
                                           format_penalty, reasoning_len, answer_len, answer_to_grade)
    return None


# ────────────────────────────────────────────────────────────────────────────
# REWARD-DICT NORMALISATION — final-step guarantee that every row written to
# the rollout dump has the same key set, whichever eval_mode produced it.
# ────────────────────────────────────────────────────────────────────────────
# Why this exists: the qa/conversation paths produce one shape; the four
# benchmark scorers add mode-specific extras (`reward/correct`,
# `reward/numeric_abs_diff`, `reward/numeric_rel_diff`, `reward/medec_line_match`).
# verl's DataProto.concat asserts that the first row's non_tensor_batch key set
# is a superset of every later row's — heterogeneous batches break that assert
# unless every row is padded to the union schema. We pad here, in one place,
# so callers never have to think about it.
#
# Defaults match the "no signal" choice for each key (zeros for numerics,
# empty string for eval_mode if somehow unset). Existing values are NEVER
# overwritten; the function only fills gaps.

_REWARD_UNION_DEFAULTS: dict = {
    # Length budget penalty (qlcm.rewards.length_penalty)
    **_length_penalty.UNION_DEFAULTS,
    # Common across every scorer
    "score":                     0.0,
    "reward/judge_score":        0.0,
    "reward/raw_score":          0.0,
    ...
    "reward/eval_mode":          "",
    # Bench-only — mcq / numeric (deterministic exact-match scorers)
    "reward/correct":            0,
    # Bench-only — numeric (medcalc_bench)
    "reward/numeric_abs_diff":   0.0,
    "reward/numeric_rel_diff":   0.0,
    # Bench-only — medec_hybrid (medec)
    "reward/medec_line_match":   0.0,
    # Coding-only — icd_multi_label (precision / recall / F1 over predicted code set)
    "reward/precision":          0.0,
    "reward/recall":             0.0,
    "reward/f1":                 0.0,
    # Coding-only — snomed_single_label (SCT-ID exact / label-fallback signal)
    "reward/sct_id_match":       0.0,
    "reward/sct_label_match":    0.0,
    # Dump-only — the row's ground truth, serialised to a string. Verl's
    # fully-async rollout-dump path can't recover `gts` from the TQ
    # (`reward_model` doesn't survive the rollouter→TQ hop), so we inject the
    # ground truth into a `reward/*` key — those DO end up in the JSONL dump.
    "reward/gold":               "",
}
...
def _serialize_gold(ground_truth) -> str:
    ...
    - str: kept as-is (already JSON or a plain label).
    - dict / list: json.dumps with ensure_ascii=False and default=str fallback.
    - None / anything else: empty string (won't surface as a useful value but
      also won't crash the dump path).
    """
...
def _normalize_reward_dict(out) -> dict:
    """Pad `out` to the union key set defined by `_REWARD_UNION_DEFAULTS`.

    Guarantees:
      - Every key in the union appears in the returned dict.
      - Existing values are preserved (not overwritten).
      - Non-dict / malformed inputs are still returned as a fully-defaulted
        dict with `score=0.0` so the trainer's logging path never explodes.
    """

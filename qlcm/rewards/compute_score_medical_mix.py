...
(so qa_bedrock's asyncio batching and qlcm's threadpool batching both
...
Wire in the training script with:
    REWARD_FN_PATH=/workspace/verl/qlcm/rewards/compute_score_medical_mix.py
...
from qlcm.rewards import qa_openrouter_bench as _qa  # noqa: E402
from qlcm.rewards import compute_score as _qlcm_cs  # noqa: E402
...
#                          "reward/eval_mode"}
#   qlcm compute_score: {"score", "prompt_uid", "data_source_code"}
#
...
_PADDING_DEFAULTS: dict = {
    **_qa._REWARD_UNION_DEFAULTS,
    # length budget penalty keys; every row must carry them (see the
    # DataProto key-union note above) even when the penalty is disabled.
    **_length_penalty.UNION_DEFAULTS,
    # qlcm compute_score keys (retention branch only):
    "prompt_uid":         0,
    "data_source_code":   0,
}
...
def _pad(result):
    ...
    No-ops if ``result`` isn't a dict (defensive — qa_bedrock and qlcm
...
def compute_score(*args, **kwargs):
    """Dual-signature entry point — same shape as the other qlcm dispatchers."""
    is_batch = (
...
def _compute_single(*args, **kwargs):
    ...
    if _is_medical(data_source):
        ...
        out = _pad(_qlcm_cs.compute_score(*args, **kwargs))
    ...
    _length_penalty.apply(out, kwargs.get("solution_str", args[1] if len(args) > 1 else ""))
    return out
...
def _compute_batch(*args, **kwargs):
    ...
    if other_idx:
        other_out = _qlcm_cs.compute_score(
    ...
    for i, r in enumerate(results):
        ...
        _length_penalty.apply(r, solution_strs[i])

...
Usage:
    python -m qlcm.curriculum.build_medical_qa_stage5 \\
        --output /data/abdelrahman/verl/data/qlcm/medical_qa_stage5/
...
# Source pairs
# ============================================================================
# Source paths. The originals live outside the dev container: ICD under
# /home/mbinakba/mcs-instruct-dataset (gone; mirrored at $QLCM_ICD) and SNOMED
# under /data/muhsen (present on the host but NOT mounted into verl-dev2), so
# the defaults point at container-visible locations. Every path is individually
# overridable, matching build_medical_qa_stage8.
...
_SOURCES = {
    "icd10_multilabel": {
        "train": os.environ.get("ICD_ML_TRAIN", _ICD + "/avey_icd_multi_label_rl_train_dataset.parquet"),
        "val":   os.environ.get("ICD_ML_VAL",   _ICD + "/avey_icd_multi_label_rl_test_dataset.parquet"),
        "icd":   True,
    },
    "icd10_instruction_follow": {
        "train": os.environ.get("ICD_IF_TRAIN", _ICD + "/avey_icd_if_rl_train_dataset.parquet"),
        "val":   os.environ.get("ICD_IF_VAL",   _ICD + "/avey_icd_if_rl_test_dataset.parquet"),
        "icd":   True,
    },
    "snomed_multilabel": {
        "train": os.environ.get("SNOMED_ML_TRAIN", _X + "/snomed_ml_train.parquet"),
        "val":   os.environ.get("SNOMED_ML_VAL",   _X + "/snomed_ml_test.parquet"),
        "icd":   False,
    },
    "snomed_instruction_follow": {
        "train": os.environ.get("SNOMED_IF_TRAIN", _X + "/snomed_if_train.parquet"),
        "val":   os.environ.get("SNOMED_IF_VAL",   _X + "/snomed_if_test.parquet"),
        "icd":   False,
    },
}

...
Outputs:
  - data/qlcm/medical_qa_stage4/train_coding_only.parquet  — snapshot of the
    pre-retention coding train.parquet (for easy rollback)
  - data/qlcm/medical_qa_stage4/train.parquet              — REPLACED: now contains
    the 25 000 coding rows + 6 250 retention rows = 31 250 total
  - data/qlcm/medical_qa_stage4/_retention_report.json     — per-pool counts +
...
Usage:
    python -m qlcm.curriculum.build_medical_qa_stage4_retention \\
        --existing_train data/qlcm/medical_qa_stage4/train.parquet \\
        --medical_cache  data/qlcm/medical_qa_stage3/_stats_cache.json \\
        --bench_cache    data/qlcm/medical_qa_stage4/_bench_retention_cache.json \\
        --general_dir    data/qlcm/train \\
        --medical_source data/medical_qa/train.parquet \\
        --bench_source   data/qlcm/medical_qa_stage3/train.parquet \\
        --output_dir     data/qlcm/medical_qa_stage4/
"""
...
def main():
    ...
    ap.add_argument("--medical_cache", required=True,
                    help="data/qlcm/medical_qa_stage3/_stats_cache.json (stage1 block is consumed).")
    ...
    ap.add_argument("--bench_source", required=True,
                    help="data/qlcm/medical_qa_stage3/train.parquet (the bench-row source).")

...
Usage:
    python -m qlcm.curriculum.score_histogram --stage 1
    python -m qlcm.curriculum.score_histogram --rollout_dir ~/some/path
    python -m qlcm.curriculum.score_histogram --stage 2 --buckets 10
"""
...
from qlcm.curriculum.encoding import decode_data_source
...
def main():
    ...
    ap.add_argument("--rollout_dir", default=None,
                    help="Explicit rollout dir. Defaults to $DUMPS_DIR/curriculum_stage{stage}_rollouts.")
    ...
    # $HOME differs between host (/home/<user>) and container (/root), so the
    # dump root is an absolute path, overridable with DUMPS_DIR like the shell
    # scripts.
    dumps_dir = os.environ.get("DUMPS_DIR", "/data/abdelrahman/verl/dumps/qlcm")
    rollout_dir = args.rollout_dir or os.path.join(
        dumps_dir, f"curriculum_stage{args.stage}_rollouts"
    )

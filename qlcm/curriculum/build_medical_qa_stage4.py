...
Usage:
    python -m qlcm.curriculum.build_medical_qa_stage4 \\
        ...
        --output /data/abdelrahman/verl/data/qlcm/medical_qa_stage4/
"""
...
def main():
    ...
    ap.add_argument("--icd", default=None,
                    help="ICD multi-label train parquet. OPTIONAL: omit to build a "
                         "SNOMED-only coding pool. The 32B run paired 15k SNOMED "
                         "single-label with 10k ICD multi-label; if the ICD source "
                         "is unavailable the stage still trains, on SNOMED alone.")
    ...
    icd    = pq.read_table(args.icd) if args.icd else None
    print(f"  SNOMED rows: {snomed.num_rows}")
    print(f"  ICD    rows: {icd.num_rows if icd is not None else 0}  (no --icd: SNOMED-only)")
    ...
    icd_ei_type    = icd.schema.field("extra_info").type if icd is not None else snomed_ei_type
    ...
    print(f"  ICD    extra_info: {icd_ei_type.num_fields if icd is not None else 0} fields")
    ...
    icd_r    = _reconcile(icd, unified_ei_type) if icd is not None else None
    print(f"  pool S (SNOMED) rows: {snomed_r.num_rows}   schema reconciled")
    print(f"  pool I (ICD)    rows: {icd_r.num_rows if icd_r is not None else 0}      schema reconciled")

    # ----- Phase 5: concat -----
    print("\n=== Phase 5: concat ===")
    pools = [snomed_r] + ([icd_r] if icd_r is not None else [])
    try:
        final = pa.concat_tables(pools, promote_options="default")

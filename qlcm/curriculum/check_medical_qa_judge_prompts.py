...
Usage:
    ...
    python -m qlcm.curriculum.check_medical_qa_judge_prompts
...
Env vars (for --response-source=openrouter, which is the default):
    ...
    RESPONSE_API_KEY   (required; falls back to QLCM_JUDGE_API_KEY if unset)
...
def _import_qa_bedrock_templates():
    from qlcm.rewards.qa_bedrock import (
...
def _build_openai_client():
    ...
    (OpenRouter by default). Reuses QLCM_JUDGE_API_KEY if RESPONSE_API_KEY
    ...
    api_key = (
        ...
        or os.environ.get("QLCM_JUDGE_API_KEY")
        or ""
    )
    if not api_key:
        raise RuntimeError(
            "no API key for response generation: set RESPONSE_API_KEY (or "
            "QLCM_JUDGE_API_KEY) to your OpenRouter key (or pass "

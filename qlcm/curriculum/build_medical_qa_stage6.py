"""Assemble the medical_qa STAGE-6 train + val parquets (real coding data).

Three sources, all multilabel scoring, per-source in-parquet duplication for
epoch weighting:

  mlb         — avey_mlb_train_dataset.parquet        (308   × 3 → 924)
                ICD-10 multilabel; comma-separated string gold.
  slb         — avey_icd_slb_rl_train_dataset.parquet (2 145 × 1 → 2 145)
                ICD-10 single-label; ndarray gold.
  snomed_ml   — final_ml_real.parquet                 (103   × 3 → 309)
...
Stamping:
  ICD sources (mlb / slb): task_type = "icd10_multilabel"
                            ground_truth → JSON list of code strings
  SNOMED ml:                task_type = "snomed_multilabel"
                            ground_truth pass-through (already JSON string)

Schema harmonisation: extra_info widened to the union of all 3 sources'
...
Usage:
    python -m qlcm.curriculum.build_medical_qa_stage6 \\
        --output /data/abdelrahman/verl/data/qlcm/medical_qa_stage6/
"""
...
_ICD = os.environ.get("QLCM_ICD", "/data/abdelrahman/mcs-instruct-dataset")
# Defaults point at container-visible copies: the ICD originals lived under
# /home/mbinakba (gone) and SNOMED under /data/muhsen (present on the host but
# not mounted into verl-dev2). Overridable per source, as in stages 5 and 8.
_SOURCES = {
    "mlb": {
        "train":     os.environ.get("MLB_TRAIN", _ICD + "/avey_mlb_train_dataset.parquet"),
        "val":       os.environ.get("MLB_VAL",   _ICD + "/avey_mlb_test_dataset.parquet"),
        "task_type": "icd10_multilabel",
        "epochs":    int(os.environ.get("MLB_EPOCHS", "3")),
        "gold_kind": "csv_string",
    },
    "slb": {
        "train":     os.environ.get("SLB_TRAIN", _ICD + "/avey_icd_slb_rl_train_dataset.parquet"),
        "val":       os.environ.get("SLB_VAL",   _ICD + "/avey_icd_slb_rl_test_dataset.parquet"),
        "task_type": "icd10_multilabel",
        "epochs":    int(os.environ.get("SLB_EPOCHS", "1")),
        "gold_kind": "list_like",
    },
    "snomed_ml": {
        "train":     os.environ.get("SNOMED_ML_REAL_TRAIN", _X + "/snomed_ml_real_train.parquet"),
        "val":       os.environ.get("SNOMED_ML_REAL_VAL",   _X + "/snomed_ml_real_test.parquet"),
        "task_type": "snomed_multilabel",
        "epochs":    int(os.environ.get("SNOMED_ML_REAL_EPOCHS", "3")),

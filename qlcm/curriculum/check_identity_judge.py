...
Usage:
    ...
    python -m qlcm.curriculum.check_identity_judge

    # Crank up to 10 prompts × 2 = 20 cases:
    python -m qlcm.curriculum.check_identity_judge --n 10

    # Use deepseek-v4-pro for the identity judge specifically:
    QLCM_IDENTITY_JUDGE_MODEL=deepseek/deepseek-v4-pro \\
        python -m qlcm.curriculum.check_identity_judge

Requires the same QLCM_JUDGE_* env vars as training (API base, key, model,
...
from qlcm.rewards.identity_judges import (
...
def main() -> int:
    ...
    ap.add_argument(
        "--parquet",
        default="/data/abdelrahman/verl/data/qlcm/train/identity_train.parquet",
    )
    ap.add_argument(
    ...
    override = _env_str("QLCM_IDENTITY_JUDGE_MODEL")
    default_model = os.environ.get("QLCM_JUDGE_MODEL", "<unset>")

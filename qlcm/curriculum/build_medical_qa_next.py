...
Usage:
    python -m qlcm.curriculum.build_medical_qa_next \\
        --cache /data/abdelrahman/verl/data/qlcm/medical_qa_stage2/_stats_cache.json \\
        --threshold 0.6 \\
        --conv_keep_n 5000 \\
        --output /data/abdelrahman/verl/data/qlcm/medical_qa_stage2/

Reused from elsewhere in the repo:
  - qlcm.preprocess.medical_qa_compat.transform_table  (struct → JSON string)
  - qlcm.curriculum.build_curated_subset._promote_strings_to_large
...
from qlcm.preprocess.medical_qa_compat import transform_table  # noqa: E402
...
def main():
    ...
    ap.add_argument("--qa_keep_n", type=int, default=None,
                    help="Cap the number of carried-forward medical_qa prompts in pool A. "
                         "Sampled uniformly at random (seeded) so the difficulty "
                         "distribution of the carry set is preserved rather than "
                         "skewed toward the hardest prompts — a set biased to the "
                         "hardest tail produces GRPO groups where every sample fails, "
                         "which carries no gradient. medical_conv rows in pool A are "
                         "never dropped. Default: keep all.")
    ...
    # The manifest records what the model was ROLLED OUT ON, which is not the
    # same as what we choose to train on next. --qa_keep_n trims the training
    # set only; every carried prompt was still seen, so snapshot the full set
    # here before any downsampling. Letting the downsample shrink the manifest
    # would make stage 3 treat the dropped prompts as unseen and reintroduce
    # them as fresh material.
    carry_idx_seen = list(carry_idx)
    if args.qa_keep_n is not None:
        # source_parquet is a PATH here, not a table (see _take).
        _ds = pq.read_table(source_parquet, columns=["data_source"]) \
                .column("data_source").to_pylist()
        _qa = [i for i in carry_idx if _ds[i] == "medical_qa"]
        _rest = [i for i in carry_idx if _ds[i] != "medical_qa"]
        if args.qa_keep_n < len(_qa):
            _qa = sorted(random.Random(args.seed).sample(_qa, args.qa_keep_n))
            print(f"  qa downsampled: {len(carry_idx) - len(_rest)} -> {len(_qa)} "
                  f"(--qa_keep_n={args.qa_keep_n}); {len(_rest)} non-qa rows kept")
        else:
            print(f"  --qa_keep_n={args.qa_keep_n} >= {len(_qa)} carried qa rows; keeping all")
        carry_idx = sorted(_qa + _rest)
    ...
    report = {
        ...
        "qa_keep_n_requested": args.qa_keep_n,
    ...
    manifest = {
        ...
        "seen_source_indices": sorted(set(carry_idx_seen) | set(mastered_idx)),
        "comment": (

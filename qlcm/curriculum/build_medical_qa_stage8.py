"""Assemble the medical_qa STAGE-8 train + val parquets (real coding, no retention).

Same train/val sources as stage 7 (no SLB). Difference vs stage 7: the
trainer script consumes ONLY train.parquet — no retention.parquet, no
general / medical / unmastered retention pools at all. This file is
identical to build_medical_qa_stage6.py's mlb+snomed_ml pools except for the
output dir and the per-source epoch counts (stage 7 is not part of this
pipeline; stage 8 branches off stage 6)
default; the schema, ground-truth normalisation, and epoch-duplication
logic carries over verbatim.

Two sources, both multilabel scoring, per-source in-parquet duplication for
epoch weighting:

  mlb         — avey_mlb_train_dataset.parquet (308 × 2 →  616)
                ICD-10 multilabel; comma-separated string gold.
  snomed_ml   — final_ml_real.parquet          (103 × 5 →  515)
...
Usage:
    python -m qlcm.curriculum.build_medical_qa_stage8 \\
        --output /data/abdelrahman/verl/data/qlcm/medical_qa_stage8/
"""
...
# Source paths are env-overridable. The originals lived under
# /home/mbinakba/mcs-instruct-dataset (deleted) and /data/muhsen (not mounted in
# the dev container), so the defaults point at the container-visible copies
# staged in data/qlcm_external/. Epoch counts are the in-parquet duplication
# factors — stage 8 has very few unique real rows (308 mlb, 103 snomed_ml), so
# these are the main lever on how much the finisher actually sees.
_X = os.environ.get("QLCM_EXTERNAL", "/data/abdelrahman/verl/data/qlcm_external")
_SOURCES = {
    "mlb": {
        "train":     os.environ.get("MLB_TRAIN", _X + "/icd_mlb_train.parquet"),
        "val":       os.environ.get("MLB_VAL",   _X + "/icd_mlb_test.parquet"),
        "task_type": "icd10_multilabel",
        "epochs":    int(os.environ.get("MLB_EPOCHS", "2")),
        "gold_kind": "csv_string",
    },
    "snomed_ml": {
        "train":     os.environ.get("SNOMED_ML_TRAIN", _X + "/snomed_ml_real_train.parquet"),
        "val":       os.environ.get("SNOMED_ML_VAL",   _X + "/snomed_ml_real_test.parquet"),
        "task_type": "snomed_multilabel",
        "epochs":    int(os.environ.get("SNOMED_ML_EPOCHS", "5")),

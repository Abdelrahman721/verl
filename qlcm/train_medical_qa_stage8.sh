#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-8 dataset (NO RETENTION).
# Multilabel-only focus on REAL clinical-note coding data — ICD-10 multilabel
# and SNOMED CT multilabel (no SLB). No retention.parquet — pure coding train.
#
# Self-contained experiment. Branches directly off STAGE 6 — there is no
# stage 7 in this pipeline (see README).  Differences from stage-6:
#   - retention.parquet dropped from TRAIN_FILES. The trainer sees ONLY the
#     coding train parquet. No general / medical_qa / unmastered rehearsal.
#     This is the FINAL stage of the pipeline — its checkpoint is the one
#     that scored best in the 32B run.
#   - Datasets:       data/qlcm/medical_qa_stage8/train.parquet     (1 131 rows; per-source duplication for epochs)
#                     data/qlcm/medical_qa_stage8/val.parquet       (145 rows, take-all)
#
#                       train.parquet pools (1 131, in-parquet epoch duplication):
#                         mlb (avey_mlb_train, real ICD multilabel)        308 × 2 =   616
#                         snomed_ml (final_ml_real, real SNOMED multilabel) 103 × 5 =   515
#                       Both stamped:
#                         mlb             → task_type=icd10_multilabel    → icd_scorer
#                         snomed_ml       → task_type=snomed_multilabel   → snomed_ml_scorer
#
#                       val.parquet pools (145, take-all):
#                         mlb         77
#                         snomed_ml   68
#
...
#                       Stage 8 only ever hits the CODING branch — the
#                       MEDICAL/BENCH and GENERAL RETENTION branches receive
#                       zero rows. The judge env blocks below are still
#                       exported for consistency with the other stage runners,
#                       but the OpenRouter QA / QLCM judges are never
#                       called this stage.
#   - Judges:         Stage-8 has NO IF rows and NO retention rows at all.
#                       Both the SNOMED-IF scorer and the qa_openrouter_bench
#                       judge path are unexercised. Setting IF_USE_LLM_JUDGES=0
#                       has no effect either way.
#
# Inherits cluster layout, sampling, KL-in-reward regime, batching, overlong
# buffer, PPO clip / loss, vLLM, FSDP and offload from the stage-6 runner.
#
...
# ---- data ----
# Stage 8 = pure coding, no retention. TRAIN_FILES has a single parquet so
# no schema harmonisation is needed across train files.
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage8/train.parquet]"}
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage8/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-QLCM}
EXP_NAME=${EXP_NAME:-medical-qa-stage8-coding}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage8_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage8_val}

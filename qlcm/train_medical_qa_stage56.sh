#!/usr/bin/env bash
# Fully-async GRPO training on the MERGED stage 5 + 6 coding dataset.
#
# Stages 5 and 6 are combined into one run. Their training scripts were already
# hyperparameter-identical (same reward fn, prompt/response budget, KL regime,
# batching, topology) — only TRAIN_FILES, VAL_FILES and EXP_NAME differed — so
# merging is a data-level change, not a config one.
#
#   - Datasets:  data/qlcm/medical_qa_stage56/train.parquet     (38 498 rows)
#                  stage-5 synthetic coding      35 120
#                    snomed_ml 10 000 · snomed_if 10 000 · ml 9 828 · if 5 292
#                  stage-6 REAL clinical notes    3 378
#                    slb 2 145 · mlb 924 (308x3) · snomed_ml 309 (103x3)
#                data/qlcm/medical_qa_stage56/retention.parquet
#                  built from THIS run's stage-4 rollouts (stage-5 recipe)
#                data/qlcm/medical_qa_stage56/val.parquet       (2 018 rows)
#
# WHAT MERGING COSTS YOU
#   In the original curriculum stage 6 existed to pivot onto REAL clinical notes
#   after the synthetic coding tasks had been learned. Merged, the real data is
#   3 378 of 38 498 rows — 8.8%, and only ~2 556 of those are unique before the
#   in-parquet epoch duplication. The synthetic pool outnumbers it ~10:1, so the
#   real-note signal is heavily diluted relative to running the stages in
#   sequence. Raise the stage-6 epoch factors if that matters; see README.
#
#   Stage-6 retention is NOT carried over. It was derived from stage-5 rollouts,
#   which a merged run never produces. Its content (if / snomed_if carry-forward)
#   is already present in the merged TRAIN pool anyway.
#
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage56/train.parquet,${DATA_DIR}/medical_qa_stage56/retention.parquet]"}
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage56/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-QLCM}
EXP_NAME=${EXP_NAME:-medical-qa-stage56-coding}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage56_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage56_val}

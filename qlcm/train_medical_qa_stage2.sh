#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-2 dataset.
# Medical-only (no curated retention mix) — single judge endpoint.
#
# Self-contained experiment — NOT the same as train_fully_async_no_math.sh.
# Differences from no_math:
#   - Dataset:        data/qlcm/medical_qa_stage2/train.parquet
#                       (curriculum-built mix of carry-forward + unseen-conv
#                        + carve_clinical + carve_lay rows, all in the
#                        medical_qa_compat schema)
...
# ---- data ----
# Medical-only training mix — the stage-2 curriculum parquet, no retention.
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage2/train.parquet]"}
VAL_FILES=${VAL_FILES:-"[${MEDICAL_QA_SRC}/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-QLCM}
EXP_NAME=${EXP_NAME:-medical-qa-stage2}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage2_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-/data/abdelrahman/verl/dumps/qlcm/medical_qa_stage2_val}
...
# ---- reward dispatcher ----
# Medical-only run, so we point directly at qa_openrouter rather than the
# qlcmed-source dispatcher (compute_score_medical_mix.py). No retention
# rows means no need for the retention branch or its key-padding logic.
REWARD_FN_PATH=${REWARD_FN_PATH:-/workspace/verl/qlcm/rewards/qa_openrouter.py}

...
Env vars:
    QLCM_JUDGE_API_BASE    — required, e.g. http://127.0.0.1:8000/v1
    QLCM_JUDGE_API_KEY     — default "EMPTY"
    QLCM_JUDGE_MODEL       — required, e.g. Qwen/Qwen2.5-7B-Instruct
    QLCM_JUDGE_TIMEOUT     — request timeout seconds (default 60)
    QLCM_JUDGE_MAX_RETRIES — exponential backoff attempts (default 3)
    QLCM_JUDGE_MAX_TOKENS  — judge response cap (default 512)
    QLCM_JUDGE_TEMPERATURE — sampling temperature (default 0.0)
...
because a plain vLLM server rejects the unknown ``provider`` body field):
    QLCM_JUDGE_PROVIDER_ONLY       — comma-separated provider slugs to pin
                                         routing to, e.g. "baidu". Empty = let
                                         OpenRouter pick from all of them.
    QLCM_JUDGE_PROVIDER_IGNORE     — comma-separated slugs to exclude.
    QLCM_JUDGE_REQUIRE_PARAMETERS  — 1 (default) routes only to providers
    ...
    QLCM_JUDGE_ALLOW_FALLBACKS     — 0 to hard-fail instead of falling back
...
_resolved = {
    "API_BASE": os.environ.get("QLCM_JUDGE_API_BASE"),
    "MODEL": os.environ.get("QLCM_JUDGE_MODEL"),
    "DISABLE_THINKING": os.environ.get("QLCM_JUDGE_DISABLE_THINKING"),
    "DISABLE_REASONING": os.environ.get("QLCM_JUDGE_DISABLE_REASONING"),
    "FORCE_JSON": os.environ.get("QLCM_JUDGE_FORCE_JSON"),
    "TEMPERATURE": os.environ.get("QLCM_JUDGE_TEMPERATURE"),
    "PROVIDER_ONLY": os.environ.get("QLCM_JUDGE_PROVIDER_ONLY"),
    "PROVIDER_IGNORE": os.environ.get("QLCM_JUDGE_PROVIDER_IGNORE"),
    "HAS_KEY": bool(os.environ.get("QLCM_JUDGE_API_KEY")),
}
...
def _provider_block(api_base: Optional[str]) -> dict:
    ...
    only = _env_list("QLCM_JUDGE_PROVIDER_ONLY")
    if only:
        provider["only"] = only
    ...
    order = _env_list("QLCM_JUDGE_PROVIDER_ORDER")
    if order:
        provider["order"] = order

    ignore = _env_list("QLCM_JUDGE_PROVIDER_IGNORE")
    if ignore:
    ...
    if os.environ.get("QLCM_JUDGE_REQUIRE_PARAMETERS", "1") != "0":
        provider["require_parameters"] = True

    allow_fallbacks = os.environ.get("QLCM_JUDGE_ALLOW_FALLBACKS")
    ...
    sort = os.environ.get("QLCM_JUDGE_PROVIDER_SORT")
    if sort:
        provider["sort"] = sort
...
def _get_client():
    ...
    api_base = os.environ.get("QLCM_JUDGE_API_BASE")
    api_key = os.environ.get("QLCM_JUDGE_API_KEY", "EMPTY")
    model = os.environ.get("QLCM_JUDGE_MODEL")
    timeout = _env_float("QLCM_JUDGE_TIMEOUT", 60.0)

    if not api_base or not model:
        raise JudgeUnavailable(
            "QLCM_JUDGE_API_BASE and QLCM_JUDGE_MODEL must be set"
        )
    ...
    *,
    ...
    Two modes:
      ...
      validate=callable    → parses the response with
                             ...
                             ``QLCM_JUDGE_MAX_RETRIES`` attempts and raise
                             JudgeUnavailable).

    ``model`` overrides the default ``QLCM_JUDGE_MODEL`` for this call
    ...
    ``QLCM_JUDGE_FORCE_JSON=0`` or by passing ``response_format={}``.
    ...
    with ``QLCM_JUDGE_REQUIRE_PARAMETERS=1`` on OpenRouter so the request
    can't be routed to a provider that would ignore it.
    """
    client, default_model = _get_client()
    effective_model = model or default_model
    max_tokens = max_tokens or _env_int("QLCM_JUDGE_MAX_TOKENS", 512)
    max_retries = _env_int("QLCM_JUDGE_MAX_RETRIES", 3)
    if temperature is None:
        temperature = _env_float("QLCM_JUDGE_TEMPERATURE", 0.0)

    if response_format is None and os.environ.get("QLCM_JUDGE_FORCE_JSON", "1") != "0":
    ...
    if os.environ.get("QLCM_JUDGE_DISABLE_THINKING", "1") != "0":
    ...
    # rewards too, so opt in explicitly (QLCM_JUDGE_DISABLE_REASONING=1).
    if os.environ.get("QLCM_JUDGE_DISABLE_REASONING", "0") != "0":
        extra_body["reasoning"] = {"enabled": False}

    provider = _provider_block(os.environ.get("QLCM_JUDGE_API_BASE"))
    if provider:

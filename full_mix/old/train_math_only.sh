#!/usr/bin/env bash
# Per-domain diagnostic: train GRPO on ONLY the math subset.
# Used to compare response_length/mean and clip_ratio against the other
# single-domain runs (ifeval/chat/safety) to see which domain is driving
# the 16k-token truncation rate in full_mix training.
#
# Identical to full_mix/train.sh except:
#   - TRAIN_FILES: only math_train.parquet
#   - VAL_FILES:   only gsm8k_eval.parquet + math500_eval.parquet
#   - EXP_NAME and dump dirs are domain-specific
...
TRAIN_FILES="[${DATA_DIR}/train/math_train.parquet]"
VAL_FILES="[${DATA_DIR}/eval/gsm8k_eval.parquet,${DATA_DIR}/eval/math500_eval.parquet]"

# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-RL-Exps}
EXP_NAME=${EXP_NAME:-full-mix-math-only}
...
# ---- judge (unused for math-only, but harmless to export) ----
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.rollout_data_dir=$HOME/verl_dumps/full_mix_math_only_rollouts \
  trainer.validation_data_dir=$HOME/verl_dumps/full_mix_math_only_val \
  "$@"

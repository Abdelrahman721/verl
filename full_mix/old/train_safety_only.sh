#!/usr/bin/env bash
# Per-domain diagnostic: train GRPO on ONLY the safety subset.
# No rule-based eval exists for safety (user does their own). VAL_FILES is a
# placeholder; eval is disabled via +trainer.val_before_train=False and a
# high test_freq.
#
# The judge IS still used to compute training rewards for safety rollouts —
# make sure FULL_MIX_JUDGE_* env vars point at a reachable endpoint.
...
TRAIN_FILES="[${DATA_DIR}/train/safety_train.parquet]"
...
EXP_NAME=${EXP_NAME:-full-mix-safety-only}
...
# ---- judge (required — safety rewards call the judge during training) ----
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.rollout_data_dir=$HOME/verl_dumps/full_mix_safety_only_rollouts \
  trainer.validation_data_dir=$HOME/verl_dumps/full_mix_safety_only_val \
  "$@"

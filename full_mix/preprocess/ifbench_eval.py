"""Preprocess 100 samples of allenai/IFBench_test for VERL evaluation."""
...
def main():
    ...
    out_path = os.path.join(out_dir, "ifbench_eval.parquet")
    ds = build(args.local_path, n=args.n)
    ds.to_parquet(out_path)
    print(f"[ifbench_eval] wrote {len(ds)} rows to {out_path}")

    with open(os.path.join(out_dir, "ifbench_eval_example.json"), "w") as f:

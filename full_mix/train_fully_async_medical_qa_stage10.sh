#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-10 dataset (DDx ranking only).
# Single-task experiment — all rows are ranked differential-diagnosis prompts
# scored by hazem's `ddx_rank` LLM-adjudicated F1 + NDCG reward.
#
# Self-contained experiment. Differences from stage-8:
#   - Data + reward are local copies (from /data/hazem/medical-data-gen/,
#     copied into this repo so nothing points at hazem's tree at runtime).
#     No retention, no schema harmonisation.
#   - Datasets:       train: data/medical_qa_stage10/train.parquet
#                            (1 188 rows, all data_source=medical_benchmark_ddx,
#                             task_type=ddx_rank)
#                     val:   data/medical_qa_stage10/val.parquet
#                            (50 rows, same shape)
#   - Reward fn:      full_mix/rewards/ddx_rank.py  (standalone, stdlib + openai)
#                       Single-route ddx_rank scorer. Routes when
#                       extra_info.eval_mode/task_type/helm_scenario == "ddx_rank"
#                       OR data_source == "medical_benchmark_ddx". Everything else
#                       is returned a neutral 0.0 reward (not relevant here — all
#                       rows hit the ddx_rank path).
#                       LLM-judged via OpenRouter:
#                         QA_JUDGE_OPENROUTER_API_KEY  (falls back to OPENROUTER_API_KEY)
#                         QA_JUDGE_MODEL               (default openai/gpt-5.4-mini)
#                         QA_JUDGE_MAX_TOKENS, QA_JUDGE_CONCURRENCY, etc.
#                       Optional knobs: QA_DDX_F1_WEIGHT (0.7), QA_DDX_NDCG_WEIGHT (0.3),
#                       QA_DDX_TOP3_PENALTY (0.05).
#
# Inherits cluster layout, sampling, KL-in-reward regime, batching, overlong
# buffer, PPO clip / loss, vLLM, FSDP and offload from the stage-9 runner.
#
...
# ---- data ----
# Stage 10 = DDx-rank only. Train + val are local copies (copied verbatim from
# /data/hazem/medical-data-gen/processed/rl/{general312,ddx}/v1/). No
# harmonisation needed; both parquets already share a canonical schema with
# data_source=medical_benchmark_ddx + task_type=ddx_rank.
DATA_DIR=${DATA_DIR:-/data/abdelrahman/verl/data}
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage10/train.parquet]"}
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage10/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-RL-Exps}
EXP_NAME=${EXP_NAME:-medical-qa-stage10-ddx}
...
TRAIN_MINI_BSZ=${TRAIN_MINI_BSZ:-4}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage10_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage10_val}
...
REWARD_FN_PATH=${REWARD_FN_PATH:-/workspace/verl/full_mix/rewards/ddx_rank.py}

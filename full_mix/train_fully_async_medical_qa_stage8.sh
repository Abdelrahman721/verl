...
#
# Self-contained experiment. Differences from stage-7:
#   - retention.parquet dropped from TRAIN_FILES. The trainer sees ONLY the
#     1 233-row coding train. No general / medical_qa / unmastered rehearsal.
#     Useful as a baseline to isolate the effect of retention.
#   - Datasets:       data/medical_qa_stage8/train.parquet     (1 131 rows; per-source duplication for epochs)
#                     data/medical_qa_stage8/val.parquet       (145 rows, take-all)
#
...
#                       but the OpenRouter QA / FULL_MIX judges are never
#                       called this stage.
...
# buffer, PPO clip / loss, vLLM, FSDP and offload from the stage-7 runner.
#
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage8_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage8_val}

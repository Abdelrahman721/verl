#!/usr/bin/env bash
# One-step-off-policy GRPO MINUS the math domain.
#
# Combines the speedup of async rollout/train (see train_one_step_off.sh) with
# the math-excluded training mix (see train_no_math.sh). Math eval sets are
# kept in VAL_FILES for observability.
#
...
# Keep math eval sets for observability.
...
EXP_NAME=${EXP_NAME:-full-mix-no-math-one-step-off}
...
N_SAMPLES_PER_PROMPT=${N_SAMPLES_PER_PROMPT:-15}
...
python3 -m verl.experimental.one_step_off_policy.main_ppo \
  ...
  trainer.rollout_data_dir=$HOME/verl_dumps/full_mix_no_math_rollouts_osop \
  trainer.validation_data_dir=$HOME/verl_dumps/full_mix_no_math_val_osop \
  ${RAY_ENV_FLAGS} \
  "$@"

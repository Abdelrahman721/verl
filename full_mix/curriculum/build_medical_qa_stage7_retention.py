"""Build the stage-7 retention pool.

Two pools per user spec (seed family = 7051), total target = 1 000 rows:

  1. General           (400 rows, seed=7051)
        Safety = 50 % of general = 200 rows (largest-remainder rounded).
        Chat + IF = 200 rows split proportional to source size.
        Sources: data/full_mix/train/{chat_with_baseline,ifeval,safety}_train.parquet

  2. Medical_qa UNSEEN (600 rows, seed=7052)
        UNSEEN vs medical_qa_rollouts (stage-3 cache: 283 749 unseen indices).
        Source: data/medical_qa/train.parquet

No stage-5 unmastered pool — stage 7 retention is purely general + medical.

Output:
  data/medical_qa_stage7/retention.parquet
  data/medical_qa_stage7/_retention_report.json
...
Reuses verbatim from earlier builds:
  ...
  - _sample_general / _sample_medical seed pattern              (stage-6 retention)

Usage:
    python -m full_mix.curriculum.build_medical_qa_stage7_retention \\
        --output_dir /data/abdelrahman/verl/data/medical_qa_stage7/
"""
...
# Canonical prompt struct order — must match the stage-7 train build so both
...
def main():
    ...
    ap.add_argument("--general_budget", type=int, default=400)
    ap.add_argument("--medical_budget", type=int, default=600)
    ap.add_argument("--seed",           type=int, default=7051)
    args = ap.parse_args()

    print(f"[build_medical_qa_stage7_retention]  seed={args.seed}")
    ...
    grand_total = pool_sizes["general"] + pool_sizes["medical"]
    ...
    report = {
        "params": {
            ...
            "seed":            args.seed,
            "seed_offsets":    {"general": 0, "medical": 1},
        },

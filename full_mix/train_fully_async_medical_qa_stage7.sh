#!/usr/bin/env bash
# Fully-async GRPO training on the Medical QA STAGE-7 dataset (+retention).
# Multilabel-only focus on REAL clinical-note coding data — ICD-10 multilabel
# and SNOMED CT multilabel (no SLB). Retention is a small general + medical_qa
# pool (no coding-unmastered carry-forward).
#
# Self-contained experiment. Differences from stage-6:
#   - SLB dropped from both train and val.
#   - Retention rebuilt from scratch (1 000 rows, general + medical only).
#   - Datasets:       data/medical_qa_stage7/train.parquet     (1 233 rows; per-source duplication for epochs)
#                     data/medical_qa_stage7/retention.parquet (1 000 rows)
#                     data/medical_qa_stage7/val.parquet       (145 rows, take-all)
#                       Both train parquets share a unified 31-field
#                       extra_info struct so HF datasets can concat-load
#                       them in one shot.
#
#                       train.parquet pools (1 233, in-parquet epoch duplication):
...
#
#                       retention.parquet pools (1 000, seed=7051):
#                         general (chat/ifeval/safety, safety=50%)             400
#                         medical_qa UNSEEN                                    600
#
...
#   - Judges:         Stage-7 has NO IF rows in train or retention (no
#                       stage-5 unmastered carry-forward). The SNOMED-IF
#                       scorer is therefore unexercised — `IF_USE_LLM_JUDGES`
#                       can be left at 1 (no-op) or set to 0. The judge env
#                       blocks are kept verbatim so the same secrets.env
#                       works across stages.
#
...
TRAIN_FILES=${TRAIN_FILES:-"[${DATA_DIR}/medical_qa_stage7/train.parquet,${DATA_DIR}/medical_qa_stage7/retention.parquet]"}
VAL_FILES=${VAL_FILES:-"[${DATA_DIR}/medical_qa_stage7/val.parquet]"}


# ---- experiment metadata ----
PROJECT_NAME=${PROJECT_NAME:-RL-Exps}
EXP_NAME=${EXP_NAME:-medical-qa-stage7-coding}
...
TRAIN_MINI_BSZ=${TRAIN_MINI_BSZ:-8}
...
ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage7_rollouts}
VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-$HOME/verl_dumps/medical_qa_stage7_val}

...
def _score_one(data_source: str, solution_str: str, ground_truth, extra_info) -> float:
    ...
    Two-step pipeline:
      ...
      2) GLOBAL IDENTITY GATE (OFF by default; QLCM_IDENTITY_GATE=1 enables):
         ...
         positively claims a conflicting identity, the domain reward is multiplied
    ...
    try:
        if base in _IDENTITY_SOURCES:
            ...
            # Reference-aligned identity scorer: grades the rollout against the
            # gold response for this prompt. It strips <think> itself and reads
            # extra_info['user_prompt']. Uses the same QLCM_JUDGE_* judge as
            # chat/safety; QLCM_IDENTITY_JUDGE_MODEL can route it to a stronger
            # model. Fails CLOSED — an unreachable judge scores every identity
            # row 0.0, which looks like a model that never gets it right.
...
# and zeroed. Identity rows are unaffected — they were scored by

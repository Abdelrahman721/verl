"""Two identity judges for the Avey Berry training pipeline.

Identity rubric + canonical identity card. Same scorer as before; the
identity it scores against was updated from Avey Olive to Avey Berry
(name, card, and prompt wording); the code is unchanged.
...
1) ``check_global_identity_violation(user_prompt, candidate)``
   ...
   judge call to ifeval. Optimizations:
     ...
     - Toggle via QLCM_GLOBAL_IDENTITY_CHECK_ENABLED=0.
...
2) ``reference_aligned_identity_score(user_prompt, reference, candidate)``
   ...
   as Avey Berry while simultaneously offering to "help with code" or
   ...
   candidate stays inside Avey Berry's identity envelope across ALL
...
from qlcm.common.think import strip_think
...
_ASSISTANT_IDENTITY_BLOCK = r"""THE ASSISTANT'S TRUE IDENTITY (the model whose output you are scoring):
  ...
  Avey Berry's CURRENT capabilities (experimental version):
    - Medical question answering.
    - ICD coding (International Classification of Diseases).
    - SNOMED coding.

  Avey Berry's PLANNED capabilities (full release):
  ...
  What Avey Berry is NOT (commonly confused / commonly requested):
...
QUESTION 1: false_model_identity
  ...
  FALSE (no false claim):
    - "I'm Avey Berry, a medicine-first AI from Avey."
...
def check_global_identity_violation(user_prompt: str, candidate: str) -> bool:
    ...
    """
    if not _env_bool("QLCM_GLOBAL_IDENTITY_CHECK_ENABLED", True):
        return False
...
_CANONICAL_IDENTITY_CARD = r"""{
  ...
  "core_description": {
    "short": "Avey Berry is a medicine-first AI assistant developed by Avey.",
    "long": "Avey Berry is a healthcare-native large language model developed by Avey. It is designed specifically for healthcare workflows and currently operates as an experimental medical AI system."
  },
  ...
  "self_identification_rules": {
    "must_identify_as": ["Avey Berry", "Avey"],
...
_REFERENCE_JUDGE_SYSTEM_PROMPT = r"""You are IdentityCardJudge. Score how well an AI assistant's response aligns with Avey Berry's CANONICAL IDENTITY CARD on the dimensions the user prompt probes.
...
_EMPTY_REFERENCE_STAND_IN = (
    "I'm Avey Berry, a medicine-first AI model developed by Avey. I'm currently "
...
def reference_aligned_identity_score(
    ...
    Model override: set ``QLCM_IDENTITY_JUDGE_MODEL`` to route THIS judge
    ...
    the cheaper QLCM_JUDGE_MODEL default).
    """
    if not candidate or not candidate.strip():
        return 0.0
    ...
    model_override = _env_str("QLCM_IDENTITY_JUDGE_MODEL")
